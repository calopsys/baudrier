...
1. **Vérification** : si le dark mode est déjà en place, Baudrier vous propose un menu (changer le mode par défaut, réinstaller le bouton, refaire l'audit des couleurs, placer le bouton dans l'UI, ou désinstaller).

2. **Détection du projet** : Baudrier vérifie que c'est bien un projet Next.js avec **Tailwind v4**. Pour les projets Tailwind v3, elle vous explique comment migrer d'abord.

3. **Installation de next-themes** : la bibliothèque de référence pour le dark mode en Next.js est installée.

4. **Configuration de la variante dark dans Tailwind** : Baudrier ajoute `@custom-variant dark` dans votre `globals.css`. À partir de maintenant, vous (ou Baudrier) pouvez écrire `dark:bg-black dark:text-white` sur n'importe quel composant.

5. **Audit des couleurs existantes** : Baudrier relit votre `globals.css`, identifie les tokens couleurs déjà définis, et vous propose **des variantes dark** pour chaque token (en gardant la même chaleur / saturation, mais inversée). Vous validez les propositions ou ajustez.

6. **Montage du ThemeProvider** : Baudrier ajoute le provider dans votre layout racine. Pas de flash au chargement, la classe `dark` est posée sur `<html>` avant l'hydratation.
...
8. **Placement guidé du bouton** (optionnel) : Baudrier détecte votre header / navbar / footer et vous propose **où insérer** `<ThemeToggle />` dans votre interface. Vous validez l'emplacement.
...
```
Baudrier peut le faire pour vous : *"adapte mon site au mode sombre"*. Elle relit chaque composant, propose les couleurs dark, vous validez.
{{/callout}}
