...
1. **Préflight SEO** : Baudrier vérifie d'abord que les fondations SEO classiques sont en place (metadata, sitemap, robots.txt, JSON-LD WebSite, structure HTML sémantique). Une page que Google ne peut pas indexer correctement ne sera pas non plus bien vue par les IA. Si quelque chose manque, Baudrier vous suggère `/seo` d'abord.
...
2. **Audit GEO complet** :
  ...
  - **Format Q&A et chunking** : Baudrier vérifie que votre contenu est découpé en blocs auto-suffisants (questions claires, paragraphes courts, sous-titres explicites)
...
3. **Soumission IndexNow (optionnel)** : Baudrier vous propose de configurer IndexNow, un protocole qui notifie Bing et Yandex (et indirectement ChatGPT qui s'appuie sur Bing) à chaque nouvelle publication.
...
5. **Corrections proposées et appliquées** : Baudrier ajoute le `llms.txt`, configure les schemas FAQPage, met à jour le robots.txt pour les crawlers IA, et propose les améliorations de contenu (que vous validez).
