...
- **Canonical URL + `astro.config` `site`**: a landing's canonical URL comes from the `site:` field in `<WEB_DIR>/astro.config.mjs` (Astro derives per-page canonical tags from it), not a per-page `alternates.canonical`. Flag ❌ if `site:` is still unset (the scaffold ships no `site:` key at all, only a comment explaining why - `/add-domain`, this skill's own fixes, or `/add-blog` are what add it).
