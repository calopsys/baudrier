...
if [ -f "<WEB_DIR>/src/lib/roles.ts" ]; then
  grep -E "^// baudrier:roles" "<WEB_DIR>/src/lib/roles.ts" | head -1
fi
```

Parse :
- No file → `existing_roles = []` (fresh install)
- `// baudrier:roles member, editor, moderator` → `["member", "editor", "moderator"]`
...
3. **Users auth required** : read `src/server/auth.ts` and look for the `// baudrier:auth-modes` marker.
...
- **Paid SaaS** (subscription/billing detected) → `free, pro, enterprise`
...
3. **Patch `src/server/db/schema.ts`** : add the value to `userRoleEnum`. Postgres : adding a value to an existing enum requires `ALTER TYPE user_role ADD VALUE 'nouveau';`. Run via tsx :
   ...
   ```
4. **Patch `src/lib/roles.ts`** : add the role to `ROLES` (tuple as const) and to `ROLE_LABELS`. Update the `// baudrier:roles ...` marker.
...
- **No i18n** : this harness is French-only (no `messages/`, no `useTranslations()`). The `/admin/users` page (and all admin pages generated by this skill), like the rest of the app, is hardcoded French.
