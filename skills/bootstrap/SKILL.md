...
- **NEVER INVOKE `preview_start` OR `pnpm dev` DURING THE BOOTSTRAP RUN itself.** This is scoped to the run, and for a mechanical reason: Step 2 through Step 8 execute as one long unattended sequence with no user turn in between, and a dev server never exits on its own - starting one mid-sequence would hang the skill waiting on a process that was never going to finish, not just steal focus or occupy a port. The preview is only launched AT THE VERY END, and ONLY after the user has explicitly answered "yes" to the question "Do you want me to launch the preview?" at Step 8. No "let me quickly check that it compiles by launching the preview" between 2 steps, no "let me test that the page renders well" during the bootstrap. During the run, validate with `pnpm tsc --noEmit` (typecheck) and `pnpm lint` - **never** `pnpm dev` nor `preview_start` - but treat these as a floor, not a substitute for actually running the app: they catch type errors and style issues only, never a runtime bug, a broken layout, or wrong data on the page. The real check happens once the user opts into the Step 8 preview, or in the normal local dev loop described below.
- **After bootstrap, the default loop is local.** Once `🎉 BOOTSTRAP COMPLETE` has been shown, the run-scoped prohibition above no longer applies: `pnpm dev` and `http://localhost:3000` are the normal way to keep building and testing the app, and the generated project's own `CLAUDE.md` documents this loop (including the `tryDb` database-resilience helper, `src/server/db/safe.ts`, once a database is configured). A deploy is not the default next action after a later change - reserve it for when the user explicitly asks to see something live, always confirm before deploying, and steer to the local review first if production is already published (see `skills/deploy/SKILL.md`).
