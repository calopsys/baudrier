...
credential_shape=$(echo "$shape_result" | node -e "console.log(JSON.parse(require('fs').readFileSync(0,'utf8')).shape)")
```
`credentialShape()` (CONTRACT.md §1, §2 `app-credentials.mjs`) returns `"org"` for an organization-administrator key (Cas A), or `"project"` for a key scoped to this Project alone (Cas B). This is the skill's own check, used only to decide whether to show the warning below - `setup-db.mjs` itself does not call `credentialShape()`. At its `ensureIamAccess` step, the script simply attempts to mint a dedicated IAM Application, policy, and key, and treats a permission-denied response as the Cas B signal: it mints nothing and uses `devDbCredentials()` instead - the Cas B path for the database, not a temporary fallback - to build `DATABASE_URL` from the environment's own key. With `SCW_DEFAULT_APPLICATION_ID` set, `devDbCredentials()` uses it directly and makes no IAM call at all; without it, it attempts one IAM read that a Cas B key cannot perform, and raises the typed error `needs_application_id` (see "If failure" below).
...
The script chains 6 sub-steps, in this order: preflight, create a dedicated IAM Application + policy + non-expiring API key, provision the database (and wait until ready), build `DATABASE_URL` and store it in Secret Manager, install the `pg` driver, swap the Drizzle client to `drizzle-orm/node-postgres`. IAM access is created before the database on purpose: the IAM step does not need the database to exist, and creating it first closes the window where a hard IAM failure would leave an orphan database behind (see the header comment in `setup-db.mjs`).
...
3. **Diagnose**:
   ...
   - `ensureIamAccess` failed then an IAM error creating the Application/policy/key - check the operator's Scaleway credentials have IAM rights. This only applies in Cas A (an organization-administrator key). In Cas B, with `SCW_DEFAULT_APPLICATION_ID` set, this step calls `devDbCredentials()` and never touches the IAM API; without it, `devDbCredentials()` attempts one IAM read that a Cas B key cannot perform and raises `needs_application_id`.
   - **`needs_application_id`** (Cas B only, no administrator needed): the environment's key cannot read its own IAM record, and `SCW_DEFAULT_APPLICATION_ID` is not set - the script has no other way to learn which IAM Application id to use as the database's "username". Relay this to the user, in their language:
     > ℹ️ Il manque un identifiant pour connecter la base de données : celui de l’application IAM qui porte votre clé. Récupérez-le dans la console Scaleway : IAM → Clés API → la ligne de la clé `SCW_ACCESS_KEY` → ouvrez l’application porteuse → copiez son identifiant. Ajoutez-le ensuite à l’environnement cloud, variable `SCW_DEFAULT_APPLICATION_ID`, de la même façon que les autres variables `SCW_*` (chapitre Installation du README). Démarrez ensuite une nouvelle conversation et relancez `/add-db`. Cette étape ne nécessite aucun administrateur.
