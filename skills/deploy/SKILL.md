...
This step, and Step 1's target question, do not apply to `/bootstrap`'s own closing deploy (its Step 8b) or to `/blogpost`'s two deploys (preview on `revue`, then the production publish) - the user's initial request to build the whole app, `/blogpost`'s article request (the consent for its preview), and `/blogpost`'s verdict (the consent for its production publish) already collect that explicit consent, as documented there. Both cases skip Step 0 and Step 1's target question, and enter directly at Step 2 with the target already fixed.
