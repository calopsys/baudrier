---
name: add-auth
description: Add NextAuth (Auth.js) authentication to an existing T3 project. One structured question decides the mode - admin (fixed login in env vars) or users (DB-backed email+password with signup, signin, account page, delete account, and optional forgot-password if email is configured). Credentials-only - no OAuth providers. This skill is an orchestrator - it delegates the actual implementation to `_setup-auth-admin` or `_setup-auth-users` depending on the chosen mode.
...
if [ -f "<WEB_DIR>/src/server/auth.ts" ]; then
  grep -E "^// baudrier:auth-modes" "<WEB_DIR>/src/server/auth.ts" | head -1
fi
```

Parse:
- No file OR no marker → `existing_modes = []`
- `// baudrier:auth-modes admin` → `["admin"]`
- `// baudrier:auth-modes users` → `["users"]`
- `// baudrier:auth-modes admin, users` → `["admin","users"]`
...
| 2 | `node "${CLAUDE_SKILL_DIR}/../../scripts/hash-password.mjs" --generate --length 24 --format alphanumeric` → push `ADMIN_PASSWORD_HASH_PROD` via `_push-env-vars --env production`, show the `password=` ONCE to the user. |
...
> 2. **Enable password reset** (forgot-password - only if email is configured)
...
| 2 | `_check-deps email` ; if OK, UPGRADE Section - Case C. Otherwise point to `/add-email`. |
| 3 | List the files to delete (auth.ts, password.ts, auth router, /signin /signup /dashboard /account pages, NextAuth tables in the DB). |
| 4 | Ask for clarification. |
...
> 2. **Enable password reset** (same as users case choice 2)
> 3. **Uninstall a mode** - destructive, ask for explicit confirmation and guide manually
> 4. **Something else**

### Case `["unknown"]` → pre-existing auth not recognized

> ⚠️ You already have a `src/server/auth.ts` but I do not recognize its origin (no baudrier marker). Three options: (1) you tell me what you want to do and I guide you manually; (2) you confirm that I can overwrite and start over; (3) we stop.
...
**Why is email+pwd the only option when there are users?** It is the only 0-config system that works everywhere and needs no third-party account. This harness does not offer OAuth sign-in providers.
...
2. **Push env vars**: `_push-env-vars --env production ADMIN_USERNAME=admin ADMIN_PASSWORD_HASH_DEV=<dev> ADMIN_PASSWORD_HASH_PROD=<prod>`.
3. **Extend `password.ts`** - add `getAdminPasswordHash()` from the admin template. Keep `hashPassword` + `verifyPassword`.
4. **Patch `auth.ts`**: marker → `// baudrier:auth-modes admin, users`. Import `getAdminPasswordHash` from `~/lib/password`. In `authorize()`, BEFORE the DB lookup, add the admin path:
   ```ts
