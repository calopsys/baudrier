...
- **Le local d’abord, pour la vérification de Claude Code** : lancer `pnpm dev` en arrière-plan, puis charger http://localhost:3000 depuis cette même machine (les pages s’affichent, les textes sont corrects, les parcours fonctionnent). Cette adresse est celle de la machine qui exécute Claude Code - jamais celle de l’utilisateur, qui ne peut pas l’ouvrir dans son propre navigateur. Pour montrer un résultat à l’utilisateur, voir « Déploiement » ci-dessous. `pnpm tsc --noEmit` et `pnpm lint` ne vérifient que les types et le style - ils ne disent rien sur le bon fonctionnement d’une page.
